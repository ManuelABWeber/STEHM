...
class HerbivoryApp:
    ...
    def save_feeding_units_plot(self):
        ...
        for i, (index, row) in enumerate(sorted_results.iterrows()):
            ax.barh(i, row['Grazing_units_per_1000ha'], color=colors['Grazing'], edgecolor='white', height=bar_width)
            ax.barh(i, -row['Browsing_units_per_1000ha'], color=colors['Browsing'], edgecolor='white', height=bar_width)
        ...
        legend_elements = [
            Patch(facecolor=colors['Browsing'], edgecolor='none', label='Browsing'),
            Patch(facecolor=colors['Grazing'], edgecolor='none', label='Grazing')
        ]
    ...
    def update_data_with_rais(self):
        ...
        relevant_columns = [('time_range_days', 'Coverage [days]')] + [(col, f"{col.split('_', 1)[-1].capitalize()} [number/day]") for col in self.rai_df.columns if col.startswith('RAI_')] + [
            ('Grazing_units_absolute', 'Grazing units (Absolute)'), 
            ('Browsing_units_absolute', 'Browsing units (Absolute)') 
        ]
    ...
    def RAI(self):
        ...
        herbivory_data = {
            ...
            "Grazing unit": [1.23, 0.87, 0.83, 1, 2.74, 0.14, 0.12, 0.03, 0.12, 0.01],
            "Browsing unit": [0.11, 0.16, 0.34, 1.22, 11.09, 0.22, 3.61, 3.76, 0.85, 0.25]
        }
        ...
        results['Grazing_units_absolute'] = 0.0
        results['Browsing_units_absolute'] = 0.0
        ...
        for col in results.columns:
            if col.startswith('RAI_'):
                ...
                if grazing_unit.size > 0:
                    results['Grazing_units_absolute'] += grazing_unit[0] * results[col].fillna(0)
                if browsing_unit.size > 0:
                    results['Browsing_units_absolute'] += browsing_unit[0] * results[col].fillna(0)
    ...
    def save_absolute_feeding_units_plot(self, results, output_base_path):
        ...
        for i, (index, row) in enumerate(sorted_results.iterrows()):
            ax.barh(i, row['Grazing_units_absolute'], color=colors['Grazing'], edgecolor='white', height=bar_width)
            ax.barh(i, -row['Browsing_units_absolute'], color=colors['Browsing'], edgecolor='white', height=bar_width)
    ...
    def relative_feeding_units_plot(self):
        ...
        results = results.dropna(subset=[
            'Grazing_units_per_1000ha', 
            'Browsing_units_per_1000ha'
        ])
    ...
    def output(self):
        ...
        relative_feeding_units_df['Grazing_units_per_1000ha'] = (
            relative_feeding_units_df['Grazing_units_absolute'] / 
            (relative_feeding_units_df['Herbaceous fraction'] * 
            relative_feeding_units_df['Area (ha)'])*1000
        )
        relative_feeding_units_df['Browsing_units_per_1000ha'] = (
            relative_feeding_units_df['Browsing_units_absolute'] / 
    ...
    def add_relative_feeding_units_to_table(self):
        metrics = [
            ('Grazing_units_per_1000ha', 'Grazing units [/1000ha grass]'), 
            ('Browsing_units_per_1000ha', 'Browsing units [/1000ha woody]') 
        ]
